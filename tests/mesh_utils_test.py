...
def mock_tpu_devices(
    x, y, z, dev_kind, one_device_per_chip, num_slices=1, reorder=False
):
  ...
  devices = [
      mock_tpu_device(core_on_chip, xd, yd, zd, xp, yp, zp, slice_index)
...
class MeshUtilsTest(test_util.JaxTestCase):
  ...
  def test_bounds_from_last_device(
      self, devices, one_device_per_chip, expected_bounds
  ):
    self.assertEqual(
  ...
  def test_create_device_mesh_for_nd_torus_split_axes_consistent_behavior(
      self, devices, mesh_shape, expected_assignment
  ):
    jax_devices = devices(True)
    physical_mesh = mesh_utils._get_physical_tpu_mesh(jax_devices)
  ...
  @parameterized.named_parameters(
      ...
      ('2x2_2d', mock_2x2_devices, [2, 4], [[0, 1, 2, 3], [6, 7, 4, 5]]),
      # 4 per-tray rings
      (
          '4x4_2d',
          mock_4x4_devices,
  ...
  def test_v3_create_device_mesh(
      self, devices, mesh_shape, expected_device_id_mesh
  ):
  ...
  def _assert_contiguous_submeshes(self, global_device_mesh):
    ...
    max_process_index = max(
        d.process_index for d in global_device_mesh.flatten()
    )
  ...
  def test_create_contiguous_submeshes_for_tpu_v4(self):
    ...
    for topology, mesh_shapes in mesh_utils._TRANSPOSE_TRICKS.items():
      ...
      devices = mock_tpu_devices(
          topology[0], topology[1], topology[2], v4, one_device_per_chip=True
      )
