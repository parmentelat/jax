...
def _create_device_mesh_for_nd_torus_splitting_axes(
  ...
  for logical_axis, logical_axis_size in reversed(
    ...
    for logical_axis_assignment in _enumerate_feasible_logical_axis_assignments(
      ...
      # scoring function reflecting the underlying hardward properties.
      if (
...
def _enumerate_feasible_logical_axis_assignments(
  ...
  """Yields feasible assignments for the given logical axis size.

  For a physical mesh of shape [x_1, ..., x_n], and an existing assignment on
  the physical axes [y_1, ..., y_n], this function yields all assignments as
  1-d arrays [z_1, ..., z_n], so that:
...
def _prefer_first_logical_axis_assignment(
  ...
  # Prefer not using the physical axes taken by the higher intensity logical
  # axes.
